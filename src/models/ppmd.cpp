...
int StartSubAllocator( qword SASize ) {
  ...
  mmap_size_ = t;
  int fd = open("ppm.temp", O_RDWR | O_CREAT | O_TRUNC, (mode_t)0664);
  if(fd < 0){
    exit(EXIT_FAILURE);
  }
...
STATE* rescale( PPM_CONTEXT& q, int OrderFall, STATE* FoundState ) {
  ...
  if( OrderFall || (q.Flags & 0x04)==0 ) {
    a = (sf-=EscFreq) - f0;
    a = CLAMP( uint( ( f0*q.SummFreq - (sf-1) *getStats(&q)->Freq + a-1 ) / a ), 2U, MAX_FREQ/2U-18U );
  } else {
    a = 2;
  }
...
void PPMD::ByteUpdate() {
  ...
  probs_ /= probs_.sum();
  if (counter_ % 10000 == 0) {
    int err = munmap(ppmd_model_->HeapStart, ppmd_model_->mmap_size_);
    if(err != 0) {
      exit(EXIT_FAILURE);
    }
    int fd = open("ppm.temp", O_RDWR);
    ppmd_model_->HeapStart = (byte*) mmap(NULL, ppmd_model_->mmap_size_, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0);
