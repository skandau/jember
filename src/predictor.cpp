...
void Predictor::AddPPMD() {
  AddByteModel(new PPMD::PPMD(25, 20095, manager_.bit_context_, vocab_));
}
...
void Predictor::AddDoubleIndirect() {
  ...
  for (const auto& params : model_params) {
    ...
    std::unique_ptr<Context> hash(new Sparse(manager_.words_, params));
    const Context& context1 = manager_.AddContext(std::move(hash));
    AddModel(new Match(manager_.history_, context1.GetContext(),
